<template>
    <div>
        ...
        <div class="clavier">
            <button v-for="button in [1,2,3,4,5,6,7,8,9,0]" @click="addNumber(button)" :key="button">
                {{ button }}
            </button>
            <button @click="Call()" class="call">Appeler</button>
        </div>
        <p>{{ findContactName(calls) }}</p>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        findContactName(calls) {
            let contact = this.contacts.filter(contact => contact.num === calls)
        return contact.length > 0 ? contact[0].prenom : 'Numéro Inconnu'
        }
    }
}
</script>
