<template>
    <div class="call">
        <h2>{{ call.number }}</h2>
        <p v-if="$store.state.contacts.find(contact => contact.num === call.number)">{{ findContactName(call.number) }} à {{ call.date }}</p>
        <p v-else>Inconnu à  {{ call.date }}</p>
        <button @click="addCalls()">
